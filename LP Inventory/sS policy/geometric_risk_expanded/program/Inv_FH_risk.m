    ...
    for d=1:length(D)
        for a=1:length(A)
            ...
            if(S(s)+A(a)<=M)
                ...
                THC=hc*(S(s)+A(a)-D(d))*(S(s)+A(a)>D(d));
                TSC=sc*(S(s)+A(a)<D(d))*(S(s)+A(a)<D(d));
                r(s,a,d)=TOC+THC+TSC;
            end
        end
    end
...
prob=zeros(M+1,Max_demand,M+1,Max_demand,N+1);
    ...
    for d=1:length(D)    
        for a=1:length(A)
            if(S(s)+A(a)<=M)
                for w=1:length(d)
                    if(S(j)<=S(s)+A(a))
                        prob(s,d,j,w,a)=prob1(S(s)+A(a)+1,S(j)+1);
                    end
                end
           end
        end
...
% disp('File writing');
% 
% fileID = fopen('Inv_FH_risk5.dat','w');
% 
% % M Value
